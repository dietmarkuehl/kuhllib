...
#include <sgi/stl_range_errors.h>
...
_Deque_base<_Tp,_Alloc,__bufsiz>::_M_initialize_map(size_t __num_elements)
{
  ...
  this->_M_map = this->_M_allocate_map(this->_M_map_size);
  ...
  __STL_UNWIND((_M_deallocate_map(this->_M_map, this->_M_map_size), 
                this->_M_map = 0, this->_M_map_size = 0));
                                           ...
                                           const value_type& __x)
{
  ...
  value_type __x_copy = __x;
  if (size_type(__elems_before) < __length / 2) {
    iterator __new_start = _M_reserve_elements_at_front(__n);
